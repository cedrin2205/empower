...
if ($_SESSION['user']['role'] !== 'HUMMSteacher') {
...
<div class="panel">
    ...
    $role = 'HUMMS'; // The role you want to filter
    ...
    <div class="container">
        <h1>Welcome Humms Teach</h1>
        <a href="logout.php" class="btn btn-warning">Logout</a>
    </div>
    <div class="contain">
        <form action="hummsupload.php" method="post" enctype="multipart/form-data">
        ...
        if (mysqli_num_rows($res) > 0) {
            while ($images = mysqli_fetch_assoc($res)) { ?>
                <div class="alb">
                    ...
                    <a class="delete-btn" href="hummsteacher.php?delete_image=<?=$images['image_url']?>">X</a>
                </div>
            <?php }
        }
        ?>
...
<script>
        ...
        document.addEventListener('click', function (e) {
            if (e.target && e.target.matches('.alb img')) {
                ...
                xhttp.open("GET", "hummsteacher.php?mark_as_seen=" + encodeURIComponent(imageUrl), true);
                xhttp.send();
